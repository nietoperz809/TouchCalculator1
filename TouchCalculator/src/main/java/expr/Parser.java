...
public class Parser
{
     ...
     * @throws SyntaxException if the input is unparsable
     */
    ...
    private Expr reparse() throws SyntaxException
    {
        ...
        if (token.ttype != Token.TT_EOF)
            throw error("Incomplete expression", SyntaxException.INCOMPLETE, null);
        return expr;
    }
        ...
        for (; ; )
        {
            ...
            switch (token.ttype)
            {
                ...
                default:
                    ...
                    if (token.ttype == Token.TT_WORD && token.sval.equals("xor"))
                    {
                        l = 10;
                        r = 11;
                        rator = Expr.XOR;
    ...
    static private final int[] rators1 =
            {Expr.ABS, Expr.ACOS, Expr.ASIN, Expr.ATAN, Expr.CEIL, Expr.COS, Expr.EXP, Expr.FLOOR,
                    Expr.LOG, Expr.ROUND, Expr.SIN, Expr.SQRT, Expr.TAN, Expr.FACT};
        ...
        switch (token.ttype)
        {
            ...
            case Token.TT_WORD:
            {
                ...
                if (null != allowedVariables && null == allowedVariables.get(var))
                    throw error("Unknown variable", SyntaxException.UNKNOWN_VARIABLE, null);
                nextToken();
                return var;
            }
            case '(':
            ...
            case Token.TT_EOF:
                throw error("Expected a factor", SyntaxException.PREMATURE_EOF, null);
            default:
    ...
    private void expect(int ttype) throws SyntaxException
    {
        if (token.ttype != ttype)
            throw error("'" + (char) ttype + "' expected", SyntaxException.EXPECTED,
                    "" + (char) ttype);
        nextToken();
    }
